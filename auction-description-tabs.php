...
<div id="auction-desc-tabs">
    ...
    <div id="wdm-desc-cmt-tab" class="auction-tab-container" style="display: none;">
        ...
        $comment_count = get_comments(array('post_id' => $wdm_auction->ID, 'count' => true,'status' => 'approve'));
            ...
            for ($i = 0; $i < $comment_count; $i++) {
                if ($comment_details[$i]->comment_parent == 0) {
                    ...
                    <div class='level<?= $level ?>'>
                        ...
                        <br />
                        <span class='ua_comment_reply' style='padding-left: 50px;text-decoration: none'><?php
                            comment_reply_link(array('add_below' => 'comment' . $comment_details[$i]->comment_ID, 'depth' => $level, 'max_depth' => 5), $comment_details[$i]->comment_ID, $wdm_auction->ID);
                        ...
                        for ($k = 0; $k < $comment_count; $k++) {
                            if (isset($comment_details[$k]) && isset($comment_details[$top_element]) && ($comment_details[$k]->comment_parent === $comment_details[$top_element]->comment_ID)) {
                                ...
                                <div class='level<?= $level ?>'>
                                    ...
                                    <br />
                                    <span class='ua_comment_reply'><?php
                                        comment_reply_link(array('add_below' => 'comment' . $comment_details[$k]->comment_ID, 'depth' => $level, 'max_depth' => $max_depth), $comment_details[$k]->comment_ID, $wdm_auction->ID);
    ...
    <div id="wdm-desc-msg-tab" class="auction-tab-container" style="display: none;">
        <form id="wdm-auction-private-form" action="">
            <div class="clearfix wdmua-shipping-field-wrap">
                <label for="wdm-prv-bidder-name" class="wdmua-shipping-label wdm-align-left"> <?php _e('Name', 'wdm-ultimate-auction'); ?>: </label>
